...
// local test
func InitAuthDB() (AuthDB, error) {
	connStr := config.NewConfigAuthDB().DB_DNS()

	conf, err := pgxpool.ParseConfig(connStr)
	if err != nil {
		return AuthDB{}, fmt.Errorf("error parsing DB config: %w", err)
	}
	...
	if err != nil {
		return AuthDB{}, fmt.Errorf("error creating DB pool: %w", err)
	}

	if err := pool.Ping(ctx); err != nil {
		pool.Close() // Đóng pool nếu ping thất bại
		return AuthDB{}, fmt.Errorf("error pinging DB: %w", err)
	}
