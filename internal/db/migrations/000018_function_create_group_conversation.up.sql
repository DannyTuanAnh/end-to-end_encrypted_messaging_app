create type create_group_result as (
...
returns create_group_result
language plpgsql
as $$
declare
    v_conversation_id bigint;
    v_group_name text;
    v_invalid_users bigint[];
...
begin
    ...
    into v_valid_users, v_invalid_users
    ...
    if coalesce(array_length(v_valid_users, 1), 0) < 3 then
        raise exception 'Group must have at least 3 members. Valid users: %, Invalid users: %', 
            ...
            coalesce(array_length(v_invalid_users, 1), 0);
    end if;
    ...
    return (v_conversation_id, v_group_name, v_invalid_users);
end;
$$;
