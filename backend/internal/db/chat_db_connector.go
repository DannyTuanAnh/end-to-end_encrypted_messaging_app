...
// local test
func InitChatDB() (ChatDB, error) {
	connStr := config.NewConfigChatDB().DB_DNS()

	conf, err := pgxpool.ParseConfig(connStr)
	if err != nil {
		return ChatDB{}, fmt.Errorf("error parsing DB config: %w", err)
	}
	...
	if err != nil {
		return ChatDB{}, fmt.Errorf("error creating DB pool: %w", err)
	}

	if err := pool.Ping(ctx); err != nil {
		pool.Close() // Đóng pool nếu ping thất bại
		return ChatDB{}, fmt.Errorf("error pinging DB: %w", err)
	}
